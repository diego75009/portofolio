...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      if (w <= 480 && (config as Record<string, unknown>)['mobile' + k]) v = (config as Record<string, unknown>)['mobile' + k] as string;
      else if (w <= 768 && (config as Record<string, unknown>)['tablet' + k]) v = (config as Record<string, unknown>)['tablet' + k] as string;
      else if (w <= 1024 && (config as Record<string, unknown>)['desktop' + k]) v = (config as Record<string, unknown>)['desktop' + k] as string;
      setVal(v);
    };
